...
class firstGui(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        for j in range(self.n):
            ...
            self.boxes.append(QtWidgets.QTextEdit(self))
            self.boxes[j].setTextInteractionFlags(QtCore.Qt.NoTextInteraction)
    ...
    def start_counting_emitter(self):
        ...
        if not self.threads[start_number].isRunning():
           ...
           self.start_counting_signal.emit(start_number, self.boxes[start_number].toPlainText())
    ...
    @QtCore.pyqtSlot(int, str)
    def set_box_text(self, boxNumber):
        self.boxes[boxNumber].insertPlainText(str(time.time()) + '\n')
